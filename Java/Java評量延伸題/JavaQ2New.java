...
//		Collections.sort(lottoList);
		...
		for (int i = 0; i < 5; i++) {
			int biggestNum = lottoList.get(i);
			for (int j = i + 1; j < 6; j++) {
				int thisNum = lottoList.get(j);
				if (thisNum > biggestNum) {
					biggestNum = thisNum;
				}
			}
			lottoList.set(lottoList.indexOf(biggestNum), lottoList.get(i));
			lottoList.set(i, biggestNum);
		}
